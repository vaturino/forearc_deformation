...
dx = 1 #horizontal resolution away from slabs
...
glon1 = np.concatenate((glon[slabs_lon], np.arange(0, 360+dx, dx)))
glon1 = np.sort(np.unique(np.round(glon1, decimals=1)))   #1382

glat1 = np.concatenate((glat[slabs_lat], np.arange(-90, 90+dx, dx)))
...
glatm, glonm,gdepthm = np.meshgrid( glat1, glon1, gdepth1, indexing='ij')  # 665, 1381, 67
